...
设计依据：`NEWS_ENGINE_AUDIT.md` 与当前项目已有采集、归档、OpenSearch、搜索、股票和行业基础字段
...
| `source_authority` | integer | 是 | `95` | 采集时来源权威分快照，防止以后配置变化改写历史语境 | `Source` 当前有效版本 | 投影 | 可按新版本另算，不覆盖快照 |
| `source_authority_version` | string(keyword) | 是 | `source_authority_2026q3` | 上述权威分所采用的治理版本 | `Source` | 投影 | 是 |
...
| `max_source_authority` | integer | 是 | `95` | 当前成员来源权威分最高快照 | 文档/Source | 投影 | 是 |
...
| `aliases` | array<string> | 否 | `["茅台","贵州茅台股份"]` | 别名、曾用名、品牌名等；应另保留有效期与类型的规范表 | 主数据/抽取/人工 | 主数据/规则/人工 | 是 |
...
| `source_authority` | integer 0..100 | 是 | `98` | 来源权威分；只定义字段，不在本阶段定义算法 | 来源治理 | 主数据/人工 | 可按版本重评 |
| `authority_level` | enum(keyword) | 是 | `primary_official` | `primary_official/high/standard/low/unverified` | 来源治理 | 主数据/人工 | 可按版本重评 |
| `authority_basis` | text | 是 | `国家监管机构官方网站` | 权威等级的审计说明 | 来源治理 | 人工/配置 | 可治理修正 |
| `authority_version` | string(keyword) | 是 | `source_authority_2026q3` | 权威规则/评审版本 | 来源治理 | 主数据/配置 | 是 |
...
| `health_status` | enum(keyword) | 是 | `healthy` | `healthy/degraded/down/disabled/unknown` | 运行监控 | 采集规则 | 是 |
| `last_success_at` | timestamp | 否 | `2026-08-15T06:30:00Z` | 最近成功采集时间 | 运行监控 | 采集规则 | 是 |
...
`source_authority` 是来源层属性，不是某条新闻的重要度。官方来源可以发布低重要度日常通知，普通媒体也可能率先报道高重要度事件；因此 Importance 只能把权威分作为一个可审计输入快照，而不能直接等同。
...
| `source_authority_snapshot` | integer 0..100 | 是 | `98` | 计算时采用的代表/最高或组合来源权威输入快照 | 文档/Source | 投影 | 新 revision 可更新 |
| `source_authority_version` | string(keyword) | 是 | `source_authority_2026q3` | 权威输入版本 | Source | 投影 | 新 revision 可更新 |
...
1. 通过 `Entity` 把代码解析到股票实体、发行人公司、别名和历史名称。
...
| 实体 | `news-entities-v1` | 名称解析、别名、实体检索 | 版本化更新 |
...
| 来源 | `news-sources-v1` | 来源治理、权威与健康 | 版本化更新 |
...
- `source_ids/news_count/source_count/first_source_id/primary_news_id`；
...
6. `source_authority`、`publish_time`、`collect_time` 在正式 `NewsDocument` 中必须存在。
...
- 文档标识与治理：`news_id` 显式源字段、`schema_version`、`source_native_id`、`source_authority`、`source_authority_version`、`document_type`、`language`、`authors`、`canonical_url`、`raw_archive_uri`、`content_license`、`document_status`。
...
- 分类与实体：`primary_category`、`category_codes`、`topic_tags`、分类版本/置信度、`entity_ids`、`industry_ids`、`entity_model_version`。
...
- 来源治理：`Source` 的来源类别、采集类型、原创/聚合属性、权威分与版本、地区、语言、时区、许可、预期频率和健康字段。
...
3. `source_authority` 的精确评分办法、不同栏目是否需要不同权威分，以及媒体原创识别的外部依据。
...
V1 的落点是：保留当前可靠的采集、原始归档、确定性文档 ID 和 OpenSearch 检索底座，在其上增加独立的事件层、实体关系层和版本化评估层。这样既不会破坏已有数据，也能让同一份新闻证据同时支撑去重后的全市场雷达与可解释的候选股深挖。
